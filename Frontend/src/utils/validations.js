...
export const validateEmployee = (data, originalData = null) => {
  ...
  else if (!alphaRegex.test(data.name.trim()))
    fieldErrors.name = "Name must contain only letters and spaces";

  // Validate Role
  if (!data.role.trim())
    fieldErrors.role = "Role is required";
  else if (!alphaRegex.test(data.role.trim()))
    fieldErrors.role = "Role must contain only letters and spaces";

  // Validate Department
  if (!data.department.trim())
    fieldErrors.department = "Department is required";
  else if (!alphaRegex.test(data.department.trim()))
    fieldErrors.department = "Department must contain only letters and spaces";
